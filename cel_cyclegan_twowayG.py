...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 17 13:32:54 2019
...
parser.add_argument("--n_residual_blocks_A", type=int, default=9, help="number of residual blocks in generator")
...
opt.n_epochs = 50
opt.decay_epoch= 10 
opt.n_residual_blocks_A = 1
opt.n_residual_blocks_B = 1
opt.n_cpu = 4
...
G_BA = [ GeneratorResNet(input_shape, opt.n_residual_blocks_B).cuda() for i in range(opt.no_models)]
...
if opt.epoch != 0:
    ...
    for i in range(opt.no_models):
        ...
        G_BA[i].apply(weights_init_normal)        
...
optimizer_G = torch.optim.Adam(
      itertools.chain(nn.ModuleList(G_AB).parameters(), nn.ModuleList(G_BA).parameters()),      
...
def sample_images(batches_done):
    ...
    with torch.no_grad(): 
       fake_A = G_BA[jj](real_A) # again, real_B is real_A, hence we changed real_B to real_A
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch  in enumerate(dataloader):
        ...
        real_B = batch["B"].squeeze() # this is necessary to get rid of the batch id and work with the label id, then using unsqueez to simulate the batch id in the loss 
...
#        fake = Variable(Tensor(np.zeros((real_A.size(0), *D_A.output_shape))), requires_grad=False)
        valid = torch.ones((real_A.size(0), *D_A.output_shape), requires_grad=False).cuda()
        fake = torch.zeros((real_A.size(0), *D_A.output_shape), requires_grad=False).cuda()
        ...
        for jj, idx in enumerate(classes_id):             
           loss_id_A += criterion_identity(G_BA[idx](real_A), real_A)
           loss_id_B += criterion_identity(G_AB[idx](real_B[jj].unsqueeze(dim=0)), 
                                           real_B[jj].unsqueeze(dim=0))
           
        loss_id_B = loss_id_B/ len_classes      
        loss_id_A = loss_id_A/ len_classes  
        
        loss_identity = (loss_id_A + loss_id_B) / 2

        # GAN loss        
        
        loss_GAN_AB = 0; loss_GAN_BA = 0; loss_fake_A = 0; loss_fake_B = 0         
        recov_B = torch.zeros(real_A.size(), dtype=torch.float32 ,
                                            requires_grad=True).cuda()         
        recov_A = torch.zeros(real_A.size(), dtype=torch.float32 ,
                                            requires_grad=True).cuda()         
        for jj, idx in enumerate(classes_id):   
           fake_B = G_AB[idx](real_A)    
           fake_A = G_BA[idx](real_B[jj].unsqueeze(dim=0)) # real_B images            
           loss_GAN_AB += criterion_GAN(D_B(fake_B ), valid) 
           loss_GAN_BA += criterion_GAN(D_A(fake_A ), valid)            
           recov_B += G_AB[idx](fake_A).squeeze(dim=0)
           recov_A += G_BA[idx](fake_B) 
           ''' your eyes on detach() !!! '''
        ...
        loss_GAN_AB = loss_GAN_BA/len_classes        
        loss_fake_A = loss_fake_A/len_classes        
        loss_fake_B = loss_fake_B/len_classes        
        
        loss_GAN = (loss_GAN_AB + loss_GAN_BA) / 2

        # Cycle loss                
        loss_cycle_A = criterion_cycle(recov_A, real_A)                 
        loss_cycle_B = criterion_cycle(recov_B, real_A) # we are changint real_B to real_A; as real_B == real_A, seems like magic                
        ...
        ''' to remove retain_graph from the above, we have
        to  use detach for loss_fake_A and loss_facke_B below
        ...
        loss_real_B = 0
        for jj in range_class_id:
           loss_real_B += criterion_GAN(D_B(real_B[jj].unsqueeze(dim=0)), 
                                      valid)
        loss_real_B = loss_real_B/len_classes              
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        for jj in range(opt.no_models):
                ...
                torch.save(G_BA[jj].state_dict(), 
                       "saved_models/%s/G_BA_%d%s%d.pth" % (opt.experiment_name, opt.epoch,'_',jj)) 
